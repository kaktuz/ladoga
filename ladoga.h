...
//#define LADOGA_1ADDR_LEN 10
//#define LADOGA_2ADDR_LEN 16
//#define LADOGA_3ADDR_LEN 24
//#define LADOGA_4ADDR_LEN 30
...
extern u32 ladoga_debug_level;
#define LADOGA_DEBUG(level, fmt, args...) \
do { if (ladoga_debug_level & (level)) \
  printk(KERN_DEBUG "ladoga: %c %s " fmt, \
...
//}
#else
#define LADOGA_DEBUG(level, fmt, args...) do {} while (0)
//static inline bool libipw_ratelimit_debug(u32 level)
//{
//	return false;
//}
...
#define LADOGA_ERROR(f, a...) printk(KERN_ERR "ladoga: " f, ## a)
#define LADOGA_WARNING(f, a...) printk(KERN_WARNING "ladoga: " f, ## a)
#define LADOGA_DEBUG_INFO(f, a...)   LADOGA_DEBUG(LADOGA_DL_INFO, f, ## a)
...
#define LADOGA_FRAG_CACHE_LEN 4

struct ladoga_frag_entry {
...
struct ladoga_auth {
	struct ladoga_hdr_3addr header;
	__le16 algorithm;
	__le16 transaction;
	__le16 status;
	/* challenge */
	struct ladoga_info_element info_element[0];
} __attribute__ ((packed));

struct ladoga_channel_switch {
	u8 id;
	u8 len;
	u8 mode;
	u8 channel;
...
struct ladoga_action {
	struct ladoga_hdr_3addr header;
	u8 category;
	u8 action;
	union {
		struct ladoga_action_exchange {
			u8 token;
			struct ladoga_info_element info_element[0];
		} exchange;
		struct ladoga_channel_switch channel_switch;

	} format;
} __attribute__ ((packed));

struct ladoga_disassoc {
	struct ladoga_hdr_3addr header;
	__le16 reason;
} __attribute__ ((packed));

/* Alias deauth for disassoc */
#define ladoga_deauth ladoga_disassoc

struct ladoga_probe_request {
	struct ladoga_hdr_3addr header;
	/* SSID, supported rates */
	struct ladoga_info_element info_element[0];
} __attribute__ ((packed));

struct ladoga_probe_response {
	struct ladoga_hdr_3addr header;
	__le32 time_stamp[2];
...
#define ladoga_beacon ladoga_probe_response

struct ladoga_assoc_request {
	struct ladoga_hdr_3addr header;
	__le16 capability;
...
struct ladoga_qos_data {
	struct ladoga_qos_parameters parameters;
	int active;
	int supported;
	u8 param_count;
	u8 old_param_count;
...
struct ladoga_tpc_report {
	u8 transmit_power;
	u8 link_margin;
} __attribute__ ((packed));

struct ladoga_channel_map {
	u8 channel;
	u8 map;
} __attribute__ ((packed));

struct ladoga_ibss_dfs {
	struct ladoga_info_element ie;
	u8 owner[ETH_ALEN];
	u8 recovery_interval;
	struct ladoga_channel_map channel_map[0];
};
...
struct ladoga_network {
	...
	struct ladoga_qos_data qos_data;

	/* These are network statistics */
	struct ladoga_rx_stats stats;
	u16 capability;
	...
	size_t rsn_ie_len;
	struct ladoga_tim_parameters tim;
	...
	struct ladoga_tpc_report tpc_report;

	/* IBSS DFS - mandatory if spctrm mgmt required and IBSS
	 * NOTE: This is variable length and so must be allocated dynamically */
	struct ladoga_ibss_dfs *ibss_dfs;

	/* Channel Switch Announcement - optional if spctrm mgmt required */
	struct ladoga_csa csa;

	/* Quiet - optional if spctrm mgmt required */
	struct ladoga_quiet quiet;
...
struct ladoga_geo {
	...
	u8 bg_channels;
	u8 a_channels;
	struct ladoga_channel bg[LIBIPW_24GHZ_CHANNELS];
	struct ladoga_channel a[LIBIPW_52GHZ_CHANNELS];
};

struct ladoga_device {
	struct net_device *dev;
	struct ladoga_security sec;

	/* Bookkeeping structures */
	struct ladoga_stats ieee_stats;
	struct ladoga_geo geo;

	/* Probe / Beacon management */
	struct list_head network_free_list;
	struct list_head network_list;
	struct ladoga_network *networks;
	int scans;
	int scan_age;
	...
	struct ladoga_frag_entry frag_cache[LADOGA_FRAG_CACHE_LEN];
	...
	void (*set_security) (struct net_device * dev,
			      struct ladoga_security * sec);
	netdev_tx_t (*hard_start_xmit) (struct ladoga_txb * txb,
					struct net_device * dev, int pri);
	int (*reset_port) (struct net_device * dev);
	int (*is_queue_full) (struct net_device * dev, int pri);

	int (*handle_management) (struct net_device * dev,
				  struct ladoga_network * network, u16 type);
	int (*is_qos_active) (struct net_device *dev, struct sk_buff *skb);

	/* Typical STA methods */
	int (*handle_auth) (struct net_device * dev,
			    struct ladoga_auth * auth);
	int (*handle_deauth) (struct net_device * dev,
			      struct ladoga_deauth * auth);
	int (*handle_action) (struct net_device * dev,
			      struct ladoga_action * action,
			      struct ladoga_rx_stats * stats);
	int (*handle_disassoc) (struct net_device * dev,
				struct ladoga_disassoc * assoc);
	int (*handle_beacon) (struct net_device * dev,
			      struct ladoga_beacon * beacon,
			      struct ladoga_network * network);
	int (*handle_probe_response) (struct net_device * dev,
				      struct ladoga_probe_response * resp,
				      struct ladoga_network * network);
	int (*handle_probe_request) (struct net_device * dev,
				     struct ladoga_probe_request * req,
				     struct ladoga_rx_stats * stats);
	int (*handle_assoc_response) (struct net_device * dev,
				      struct ladoga_assoc_response * resp,
				      struct ladoga_network * network);

	/* Typical AP methods */
	int (*handle_assoc_request) (struct net_device * dev);
	int (*handle_reassoc_request) (struct net_device * dev,
				       struct ladoga_reassoc_request * req);
...
/* ieee80211.c */
extern void free_ladoga(struct net_device *dev);
extern struct net_device *alloc_ladoga(int sizeof_priv);
